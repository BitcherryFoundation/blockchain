...
import (
	...
	rpcclient "github.com/hashrs/blockchain/core/consensus/dpos-pbft/rpc/lib/client"
	"github.com/hashrs/blockchain/core/consensus/dpos-pbft/types"
)

/*
HTTP is a Client implementation that communicates with a HashRs node over
JSON RPC and WebSockets.

This is the main implementation you probably want to use in production code.
There are other implementations when calling the HashRs node in-process
(Local), or when you want to mock out the server for test code (mock).

You can subscribe for any event published by HashRs using Subscribe method.
Note delivery is best-effort. If you don't read events fast enough or network is
slow, HashRs might cancel the subscription. The client will attempt to
...
func (w *WSEvents) Subscribe(ctx context.Context, subscriber, query string,
	...
	w.mtx.Lock()
	// subscriber param is ignored because HashRs will override it with
	// remote IP anyway.
