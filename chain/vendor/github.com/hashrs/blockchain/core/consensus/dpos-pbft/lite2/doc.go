...
HashRs light clients allow bandwidth & compute-constrained devices, such as
smartphones, low-power embedded chips, or other blockchains to efficiently
verify the consensus of a HashRs blockchain. This forms the basis of safe
and efficient state synchronization for new network nodes and inter-blockchain
communication (where a light client of one HashRs instance runs in another
...
NOTE: HashRs provides a somewhat different (stronger) light client model
...
https://github.com/hashrs/spec/blob/master/spec/consensus/light-client.md
for details.

## 3. Secure RPC proxy

HashRs RPC exposes a lot of info, but a malicious node could return any
...
See
https://github.com/hashrs/blockchain/core/consensus/dpos-pbft/blob/master/cmd/hashrs/commands/lite.go
for usage example.
*/
package lite
