...
import (
	...
	"github.com/hashrs/blockchain/core/consensus/dpos-pbft/abci/example/kvstore"
	...
	db "github.com/hashrs/blockchain/libs/state-db/tm-db"
)
...
func WALGenerateNBlocks(t *testing.T, wr io.Writer, numBlocks int) (err error) {
	...
	case <-time.After(1 * time.Minute):
		consensusState.Stop()
		return fmt.Errorf("waited too long for hashrs to produce %d blocks (grep logs for `wal_generator`)", numBlocks)
	}
}
