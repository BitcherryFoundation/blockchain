...
import (
	...
	pvm "github.com/hashrs/blockchain/core/consensus/dpos-pbft/privval"
	...
	"github.com/hashrs/blockchain/framework/chain-app/simapp"
...
var (
	...
	Cdc             *codec.Codec
	DefaultCLIHome  = os.ExpandEnv("$HOME/.chain.c")
	DefaultNodeHome = os.ExpandEnv("$HOME/.chain.c")
...
// InitChainer is called by HashRs to start the chain.
func (app *AppStarter) InitChainer(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
	...
	/*
		...
		functionality of hashrs. It is done here like
...
func BuildModuleBasics(chainName string, moduleBasics ...module.AppModuleBasic) {
	name := strings.ToLower(chainName)
	DefaultCLIHome = os.ExpandEnv("$HOME/." + name + ".c")
	DefaultNodeHome = os.ExpandEnv("$HOME/." + name + ".d")
...
func NewAppStarter(chainName string, logger log.Logger, db dbm.DB, moduleBasics ...module.AppModuleBasic) *AppStarter {

	BuildModuleBasics(chainName, moduleBasics...)

	Cdc = MakeCodec()

	bApp := bam.NewBaseApp(chainName, logger, db, auth.DefaultTxDecoder(Cdc))
