...
- [pruning] [/#158](https://github.com/hashrs/blockchain/framework/chain-iavl/pull/158) NodeDB constructor must provide `keepRecent` and `keepEvery` fields to define PruningStrategy. All Save functionality must specify whether they should flushToDisk as well using `flushToDisk` boolean argument. All Delete functionality must specify whether object should be deleted from memory only using the `memOnly` boolean argument.
- [dep] [\#194](https://github.com/hashrs/blockchain/framework/chain-iavl/pull/194) Update tm-db to 0.4.0 this includes interface breaking to return errors.

### IMPROVEMENTS

### Bug Fix

- [orphans] [#177](https://github.com/hashrs/blockchain/framework/chain-iavl/pull/177) Collect all orphans after remove (@rickyyangz)

## 0.12.4 (July 31, 2019)

### IMPROVEMENTS

- [\#46](https://github.com/hashrs/blockchain/framework/chain-iavl/issues/46) Removed all instances of cmn (hashrs/hashrs/libs/common)
...
- Implement LazyLoadVersion (@alexanderbez)
  ...
  without loading previous roots/versions. - see [goDoc](https://godoc.org/github.com/hashrs/blockchain/framework/chain-iavl#MutableTree.LazyLoadVersion)
...
- Use HashRs v0.30.2 and close batch after write (related pull request in HashRs: https://github.com/hashrs/blockchain/core/consensus/dpos-pbft/pull/3397)
...
- Uses new HashRs ReverseIterator API. See https://github.com/hashrs/blockchain/core/consensus/dpos-pbft/pull/2913
...
[#107]: https://github.com/hashrs/blockchain/framework/chain-iavl/pull/107
[@silasdavis]: https://github.com/silasdavis
[#96]: https://github.com/hashrs/blockchain/framework/chain-iavl/pull/96
...
[1]: https://github.com/hashrs/blockchain/framework/chain-iavl/blob/9e62436856efa94c1223043be36ebda01ae0b6fc/mutable_tree.go#L14-L21
[2]: https://github.com/hashrs/blockchain/framework/chain-iavl/blob/9e62436856efa94c1223043be36ebda01ae0b6fc/immutable_tree.go#L10-L17
...
- Change hashrs dep to ^v0.22.0 (#91)
...
- Swap `tmlibs` for `hashrs/libs`
- Remove `sha256truncated` in favour of `hashrs/crypto/tmhash` - same hash
  function but technically a breaking change to the API, though unlikely to effect anyone.

NOTE this means IAVL is now dependent on HashRs Core for the libs (since it
makes heavy use of the `db` package). Ideally, that dependency would be
abstracted away, and/or this repo will be merged into the HashRs-SDK, which is
